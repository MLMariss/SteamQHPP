...
| `trailers.py`           | `trailers.json`      | Store-trailer CDN filenames per game, so hovering a thumbnail plays the game's trailer instead of just enlarging the still. A trailer's *movie id* isn't derivable from the appid, so unlike every other bit of store art it has to be looked up and stored. Batched `GetItems`; backlog-drain, not a refresh loop. + `trailers_state.json` (queue state, not served). |
